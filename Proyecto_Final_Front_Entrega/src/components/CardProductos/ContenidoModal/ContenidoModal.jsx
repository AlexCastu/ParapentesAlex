...
const ContenidoModal = ({ data, producto, cerrarModal }) => {
  ...
  const agregarElemento = () => {
    axios
      .get(
        `https://proyectofinalback-production.up.railway.app/tienda/agregarALaCesta?id=${data._id}&categoria=${producto}`
      )
      .then((response) => {
        if (
        ...
        <div className="datosProducto">
          ...
          <div className="divBotonModal">
            <Button variant="info" onClick={funcion}>
              Añadir a la cesta{" "}
              <img src="https://img.icons8.com/ios/24/null/buy--v1.png" />
            </Button>
          </div>
        </div>
      </div>
